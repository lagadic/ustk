...
usNetworkServer::usNetworkServer(QObject *parent) : QObject(parent)
{
	...
	//init porta
	bool initPortaSucess = m_porta->init(256*1024*1024,portaFirmware,portaSettings,portaLicense,portaLut,usm,pci,highVoltage,0,64);
	if(!initPortaSucess) {
...
int portaCallbackImage(void* param, int id, int)
{
	...
	if(server->imageHeader.imageType == 1) { //post scan
		...
		beginImage = new unsigned char[portaInstance->getParam(prmBNumLines) * portaInstance->getParam(prmBNumSamples) * sizeof(unsigned char)];
