...
* @param [out] preScanImage The pre-scan image to read.
* @param [in] headerFileName The header file name to read, with header extension.
* @param [in] imageNumberInSequence The image number of the image to read in the sequence (from 0 for 1st volume stored).
*/
void usImageIo::read(usImagePreScan3D<unsigned char> &preScanImage,const std::string &headerFileName, int imageNumberInSequence)
{
  usImageIo::usHeaderFormatType headerFormat = getHeaderFormat(headerFileName);
  if(headerFormat == FORMAT_VOL) {
    //INIT
    int szHeader = sizeof(VolHeader);
    int n = 0;
    char byte;
    ...
    // OFFSET
    // to select volume to read (in .vol files multiple volumes can be stacked after the header part)
    int offset = szHeader + imageNumberInSequence * header.fpv * header.w * header.h;
