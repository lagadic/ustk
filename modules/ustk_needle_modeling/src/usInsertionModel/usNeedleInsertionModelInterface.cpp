...
bool usNeedleInsertionModelInterface::setBasePose(double tx, double ty, double tz, double thetax, double thetay, double thetaz)
{
...
{
    if(v.size()!=6) throw vpException(vpException::dimensionError, "usNeedleInsertionModelInterface::moveBase(const vpColVector&): invalid vector dimension");
    if(time<0) throw vpException(vpException::badValue, "usNeedleInsertionModelInterface::moveBase(const vpColVector&): negative time period");

    if(time<=std::numeric_limits<double>::epsilon()) return true;
    if(v.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return true;

    return this->moveBase(vpExponentialMap::direct(v, time));
}

bool usNeedleInsertionModelInterface::moveBase(double tx, double ty, double tz, double thetax, double thetay, double thetaz)
{
...
{
    if(v.size()!=6) throw vpException(vpException::dimensionError, "usNeedleInsertionModelInterface::moveBaseWorldFrame(const vpColVector&): invalid vector dimension");

    if(time<=std::numeric_limits<double>::epsilon()) return false;
    if(v.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return false;

    return this->moveBaseWorldFrame(vpExponentialMap::direct(v, time));
}

bool usNeedleInsertionModelInterface::moveBaseWorldFrame(double tx, double ty, double tz, double thetax, double thetay, double thetaz)
{
