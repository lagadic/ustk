...
usNeedleInsertionModelRayleighRitzSpline::usNeedleInsertionModelRayleighRitzSpline():
    ...
    m_tissue(),
    m_pathUpdateType(PathUpdateType::NoUpdate),
    m_pathUpdateLengthThreshold(0.001),
    m_pathUpdateMixCoefficient(0.5),
...
usNeedleInsertionModelRayleighRitzSpline::~usNeedleInsertionModelRayleighRitzSpline()
{
    if(m_needleTip != nullptr) delete m_needleTip;
}
...
{
    m_needle = model.m_needle;
    if(m_needleTip) delete m_needleTip;
    m_needleTip = model.m_needleTip->clone();
    m_needleTipType = model.m_needleTipType;
    m_tissue = model.m_tissue;
    m_stiffnessPerUnitLength = model.m_stiffnessPerUnitLength;
...
void usNeedleInsertionModelRayleighRitzSpline::loadPreset(const ModelPreset preset)
{
    switch(preset)
    {
        case ModelPreset::BiopsyNeedle :
        {
            ...
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 20000);
            break;
        }
        ...
        case ModelPreset::AbayazidRRM13 :
        {
            ...
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 35500*4*0.2/0.17);
            break;
        }
        case ModelPreset::MisraRSRO10_PlastisolA :
        {
            ...
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 4830);
            break;
        }
        case ModelPreset::RoesthuisAM12 :
        {
            ...
            t->setDiameter(d);
            t->setLength(d / tan(M_PI/180*30));
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 150000);
            break;
        }
        case ModelPreset::SteelSoftTissue :
        {
            ...
            t->setDiameter(d);
            t->setLength(d);
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 500);
            break;
        }
        case ModelPreset::SRL_ActuatedFBG :
        {
            ...
            for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) this->setStiffnessPerUnitLength(i, 35000);
            break;
        }
...
{
    if(type == m_needleTipType) return;
...
{
    if( K<=0 ) return false;
    for(unsigned int i=0 ; i<m_stiffnessPerUnitLength.size() ; i++) m_stiffnessPerUnitLength.at(i) = K;
    return true;
}
...
double usNeedleInsertionModelRayleighRitzSpline::getNeedleFreeLength(int *seg, double *param) const
{
    if(this->IsNeedleInserted())
    {
        ...
        {
            if(seg!=nullptr) *seg = -1;
            if(param!=nullptr) *param = -1;
            return 0;
        }
        ...
        if(i<nbSeg)
        {
            if(seg!=nullptr) *seg = i;
            if(param!=nullptr) *param = l;
        }
        else
        {
            if(seg!=nullptr) *seg = -1;
    ...
    else
    {
        ...
        if(param!=nullptr) *param = -1;
        double L = 0;
        for(int i=0 ; i<m_needle.getNbSegments() ; i++) L += m_needle.accessSegment(i).getParametricLength();
        return L;
    }
}
...
{
    if(!this->IsNeedleInserted()) throw vpException(vpException::fatalError, "usNeedleInsertionModelRayleighRitzSpline::getNeedleInsertionPoint: needle is not inserted");
...
{
    if(m_tissue.accessPath().getNbSegments()<1) throw vpException(vpException::fatalError, "usNeedleInsertionModelRayleighRitzSpline::getTissueInsertionPoint: needle was not inserted");

    return m_tissue.accessPath().getPoint(0);
}

bool usNeedleInsertionModelRayleighRitzSpline::getCorrespondingPathPoint(double l, int &correspondingRestIndex, double &correspondingRestParam) const
{
    correspondingRestIndex = -1;
    correspondingRestParam = -1;
    if(m_tissue.accessPath().getNbSegments()<1) return false;
    
    int needleIndex = -1;
    double needleParam = -1;
    double freeLength = this->getNeedleFreeLength(&needleIndex,&needleParam);
    if(l<freeLength || needleIndex <0 || needleParam<0) return false;
    ...
    while(currentNeedleInsertedLength < insertionLength)
    {
        ...
        if(needleIndex<m_needle.getNbSegments()-1 && currentNeedleInsertedLength + m_needle.accessSegment(needleIndex).getParametricLength() - needleParam < insertionLength)
        {
        ...
        while(restIndex<m_tissue.accessPath().getNbSegments()-1 && currentRestLength + m_restDilatationFactor.at(needleIndex)*(m_tissue.accessPath().accessSegment(restIndex).getParametricLength()-currentRestParam) < currentNeedleInsertedLength)
        {
            currentRestLength += m_restDilatationFactor.at(needleIndex)*(m_tissue.accessPath().accessSegment(restIndex).getParametricLength()-currentRestParam);
            currentRestParam = 0;
            restIndex++;
        }
...
{
    if(!this->IsNeedleInserted()) return 0;
...
bool usNeedleInsertionModelRayleighRitzSpline::setBasePose(const vpPoseVector &pose)
{
    ...
    if(m_tissue.accessSurface().getDirection().euclideanNorm()>0)
    {
        ...
        if( usGeometryTools::IsPointInFrontOfPlane(position, m_tissue.accessSurface()) ) return false;
    }
...
{
    if(P.size()!=3) throw vpException(vpException::dimensionError, "usNeedleInsertionModelRayleighRitzSpline::cutPathToPoint: invalid vector dimension");
    ...
    if(vpColVector::dotProd(P - lastPoint, lastDirection) > m_pathUpdateLengthThreshold)
    {
        ...
        {
            std::cout << "Warning usNeedleInsertionModelRayleighRitzSpline::cutPathToPoint: cut point is inconsistent with current needle state" << std::endl;
            return false;
        }
...
void usNeedleInsertionModelRayleighRitzSpline::solveSegmentsParametersSparseEigen()
{
    ...
    for(int j=0; j<nbSegEq ; j++)
    {
        ...
        {
            if(j>1) L.push_back(T(line, k, EI * pow(segLength,j+k-3)*j*(j-1)*k*(k-1)/(j+k-3)));
        }
        line++;
    }
...
#else
    throw vpException(vpException::functionNotImplementedError, "usNeedleInsertionModelRayleighRitzSpline::solveSegmentsParametersSparseEigen: not implemented without Eigen3");
#endif
}
...
void usNeedleInsertionModelRayleighRitzSpline::solveSegmentsParametersFullSparseEigen()
{
    ...
    if(trueFreeLength>=m_needle.getFullLength()) nbSegments = 1;
    ...
    for(int i=1 ; i<nbSegments-1 ; i++) m_needle.accessSegment(i).setParametricLength(segDefaultLength);
    if(nbSegments>1) m_needle.accessLastSegment().setParametricLength(m_needle.getFullLength() - m_needle.accessSegment(0).getParametricLength() - segDefaultLength*(nbSegments-2));
    ...
    for(int j=0; j<nbSegEq ; j++)
    {
        ...
        for(int k=2 ; k<nbSegCoef ; k++)
        {
            if(j>1) for(int dim=0; dim<3 ; dim++) L.push_back(T(line+dim, 3*k+dim, EI * pow(segLength,j+k-3)*j*(j-1)*k*(k-1)/(j+k-3)));
        }
        line+=3;
    }
...
void usNeedleInsertionModelRayleighRitzSpline::solveSegmentsParametersFullSparseEigenFixedLength()
{
    ...
    m_restDilatationFactor.resize(m_needle.getNbSegments(), 1);
    for(int i=0 ; i<nbSegments-1 ; i++) m_needle.accessSegment(i).setParametricLength(segDefaultLength);
    if(nbSegments>1)
    {
        double l = m_needle.getFullLength() - segDefaultLength*(nbSegments-1);
        if(l > std::numeric_limits<double>::epsilon()) m_needle.accessLastSegment().setParametricLength(l);
        else
        {
            m_needle.removeLastSegment();
            nbSegments--;
        ...
        while(lseg < segLength)
        {
            ...
            for(int j=0; j<nbSegEq ; j++)
            {
                for(int k=0 ; k<nbSegCoef ; k++)
                {
                    ...
                    for(int dim=0; dim<3 ; dim++) M[line+3*j+dim][startIndex+3*k+dim] =  c;
                }
            }
        ...
        while(lbev < bevLength)
        {
            ...
            for(int j=0; j<nbSegEq ; j++)
            {
                for(int k=0 ; k<nbSegCoef ; k++)
                {
                    ...
                    for(int dimj=0; dimj<3 ; dimj++)for(int dimk=0; dimk<3 ; dimk++)  M[line+3*j+dimj][startIndex+3*k+dimk] = coefMat[dimj][dimk];
                }
            }
    ...
    do
    {
        ...
        if(this->IsNeedleInserted() && m_tissue.accessPath().getNbSegments()>0)
        {
            ...
            if(usGeometryTools::DoesSegmentCrossPlaneDirect(m_tissue.accessPath(), pl))
            {
                ...
                double restLength = -1;
                usGeometryTools::getPlaneCurveCrossingPoint(m_tissue.accessPath(), pl, 0.1*m_needle.getOuterDiameter(), &restLength);

                for(unsigned int i=0 ; i<m_restDilatationFactor.size() ; i++) m_restDilatationFactor.at(i) = l / restLength;
            }
            else
            {
...
bool usNeedleInsertionModelRayleighRitzSpline::updatePath()
{
    switch(m_pathUpdateType)
    {
        ...
        case PathUpdateType::WithTipDirection:
        {
            ...
            if(m_tissue.accessPath().getNbSegments()>0)
            {
                this->getCorrespondingPathPoint(m_needle.getFullLength()+(m_needleTip->getTipPosition()-m_needleTip->getBasePosition()).euclideanNorm(), restIndex, restParam);
                lastPoint = m_tissue.accessPath().accessLastSegment().getEndPoint();
            ...
            else if(usGeometryTools::IsPointInFrontOfPlane(m_needleTip->getTipPosition(), m_tissue.accessSurface()))
            {
                ...
                lastPoint = usGeometryTools::projectPointOnPlane(m_needleTip->getTipPosition(), m_tissue.accessSurface(), lastDirection);
            }
            else return false;
        ...
        case PathUpdateType::WithTipMix:
        {
            ...
            if(insertionStep > m_pathUpdateLengthThreshold)
            {
                ...
                vpColVector newPoint = m_pathUpdateMixCoefficient * (lastPoint + insertionStep*m_needleTip->getTipDirection()) + (1-m_pathUpdateMixCoefficient) * m_needleTip->getTipPosition();
...
std::ostream &operator<<(std::ostream &s, const usNeedleInsertionModelRayleighRitzSpline &model)
{
    ...
    s << n << '\n';
    for(int i=0 ; i<n ; i++) s << model.m_stiffnessPerUnitLength.at(i) << '\n';
    for(int i=0 ; i<n ; i++) s << model.m_layerLength.at(i) << '\n';
...
std::ostream &operator<<=(std::ostream &s, const usNeedleInsertionModelRayleighRitzSpline &model)
{
    ...
    for(int i=0 ; i<n ; i++) s.write((char*)&(model.m_stiffnessPerUnitLength.at(i)), sizeof(double));
    for(int i=0 ; i<n ; i++) s.write((char*)&(model.m_layerLength.at(i)), sizeof(double));

    n = model.m_restDilatationFactor.size();
    s.write((char*)&(n), sizeof(int));
    for(int i=0 ; i<n ; i++) s.write((char*)&(model.m_restDilatationFactor.at(i)), sizeof(double));
...
std::istream &operator>>=(std::istream &s, usNeedleInsertionModelRayleighRitzSpline &model)
{
    ...
    model.m_stiffnessPerUnitLength.resize(n);
    model.m_layerLength.resize(n);
    for(int i=0 ; i<n ; i++) s.read((char*)&(model.m_stiffnessPerUnitLength.at(i)), sizeof(double));
    for(int i=0 ; i<n ; i++) s.read((char*)&(model.m_layerLength.at(i)), sizeof(double));

    s.read((char*)&(n), sizeof(int));
    model.m_restDilatationFactor.resize(n);
    for(int i=0 ; i<n ; i++) s.read((char*)&(model.m_restDilatationFactor.at(i)), sizeof(double));
    return s;
}
