...
{
    return new usNeedleModelBaseTip(*this);
}
...
{
    return m_basePose.getTranslationVector();
}
...
{
    return m_tipPose.getTranslationVector();
}
...
{
    if(control.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveBase(const vpColVector&, double): invalid control vector dimension, should be 6");
    if(time<=std::numeric_limits<double>::epsilon()) return;
    if(control.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return;
...
{
    if(v.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveBase(const vpColVector): invalid vector dimension, should be 6");
...
{
    if(control.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::usNeedleModelBaseWorldFrame(const vpColVector&, double): invalid control vector dimension, should be 6");
    if(time<=std::numeric_limits<double>::epsilon()) return;
    if(control.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return;
...
void usNeedleModelBaseTip::moveBaseWorldFrame(double tx, double ty, double tz, double thetax, double thetay, double thetaz)
{
    vpHomogeneousMatrix Hmotion(tx, ty, tz, thetax, thetay, thetaz);
...
{
    if(v.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveBaseWorldFrame(const vpColVector&): invalid vector dimension, should be 6");
    vpHomogeneousMatrix Hmotion(v[0],v[1],v[2],v[3],v[4],v[5]);
...
{
    if(control.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveTip(const vpColVector&, double): invalid control vector dimension, should be 6");
    if(time<=std::numeric_limits<double>::epsilon()) return;
    if(control.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return;
...
{
    if(v.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::MoveTip(const vpColVector&): invalid vector dimension, should be 6");
    vpHomogeneousMatrix Hmotion(v[0],v[1],v[2],v[3],v[4],v[5]);
...
{
    if(control.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveTipWorldFrame(const vpColVector&, double): invalid control vector dimension, should be 6");
    if(time<=std::numeric_limits<double>::epsilon()) return;
    if(control.euclideanNorm()<=std::numeric_limits<double>::epsilon()) return;
...
void usNeedleModelBaseTip::moveTipWorldFrame(double tx, double ty, double tz, double thetax, double thetay, double thetaz)
{
    vpHomogeneousMatrix Hmotion(tx, ty, tz, thetax, thetay, thetaz);
...
{
    if(v.size() != 6) throw vpException(vpException::dimensionError, "usNeedleModelBaseTip::moveTipWorldFrame(const vpColVector&): invalid vector dimension, should be 6");
    vpHomogeneousMatrix Hmotion(v[0],v[1],v[2],v[3],v[4],v[5]);
...
std::ostream &operator<<(std::ostream &s, const usNeedleModelBaseTip &needle)
{
    ...
    for(int i=0 ; i<6 ; i++) s << needle.m_basePose[i] << " ";
    s << '\n';
    for(int i=0 ; i<4 ; i++)
    {
        for(int j=0 ; j<4 ; j++) s << needle.m_worldMbase[i][j] << " ";
        s << '\n';
    }
    for(int i=0 ; i<6 ; i++) s << needle.m_tipPose[i] << " ";
    s << '\n';
    for(int i=0 ; i<4 ; i++)
    {
        for(int j=0 ; j<4 ; j++) s << needle.m_worldMtip[i][j] << " ";
        s << '\n';
    }
...
std::istream &operator>>(std::istream &s, usNeedleModelBaseTip &needle)
{
    ...
    for(int i=0 ; i<6 ; i++) s >> needle.m_basePose[i];
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s >> needle.m_worldMbase[i][j];
    for(int i=0 ; i<6 ; i++) s >> needle.m_tipPose[i];
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s >> needle.m_worldMtip[i][j];
    return s;
}
...
std::ostream &operator<<=(std::ostream &s, const usNeedleModelBaseTip &needle)
{
    ...
    for(int i=0 ; i<6 ; i++) s.write((char*)&(needle.m_basePose[i]), sizeof(double));
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s.write((char*)&(needle.m_worldMbase[i][j]), sizeof(double));
    for(int i=0 ; i<6 ; i++) s.write((char*)&(needle.m_tipPose[i]), sizeof(double));
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s.write((char*)&(needle.m_worldMtip[i][j]), sizeof(double));
...
std::istream &operator>>=(std::istream &s, usNeedleModelBaseTip &needle)
{
    ...
    for(int i=0 ; i<6 ; i++) s.read((char*)&(needle.m_basePose[i]), sizeof(double));
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s.read((char*)&(needle.m_worldMbase[i][j]), sizeof(double));
    for(int i=0 ; i<6 ; i++) s.read((char*)&(needle.m_tipPose[i]), sizeof(double));
    for(int i=0 ; i<4 ; i++) for(int j=0 ; j<4 ; j++) s.read((char*)&(needle.m_worldMtip[i][j]), sizeof(double));
    return s;
}
