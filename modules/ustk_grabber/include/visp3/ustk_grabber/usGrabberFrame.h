 ...
 */
template <class ImageType> class usGrabberFrame
{
public:
  /**
   * Constructor
...
template <class ImageType> usGrabberFrame<ImageType>::usGrabberFrame() {}

template <class ImageType> usGrabberFrame<ImageType>::~usGrabberFrame() {}

template <class ImageType>
void usGrabberFrame<ImageType>::setCommunicationInformation(
    usGrabberUltrasonix::usGrabberCommunicationInformations *informations)
{
...
template <class ImageType> void usGrabberFrame<ImageType>::grabFrame(ImageType *imageToWrite) const
{
  // Grab a single frame
  int n = 0;
  ...
  if (m_informations->m_totFrmIdx != m_informations->m_previousFrmIdx + 1) {
    throw vpException(vpException::ioError, "Error in data index: totFrmIdx = %d; previousFrmIdx = %d",
  ...
  // Copy data
  memcpy(m_informations->m_voldata + m_informations->m_frmIdx * m_informations->m_szFrm,
         m_informations->m_frmInf + sizeof(int) + sizeof(double), m_informations->m_szFrm * sizeof(unsigned char));
  ...
  } else if (m_informations->m_header.imageType == 0 && m_informations->m_header.framesPerVolume > 3) // prescan 3D
  {
    ...
    for (int i = 0; i < m_informations->m_header.height; ++i) {
      for (int j = 0; j < m_informations->m_header.width; ++j) {
        (*imageToWrite)(i, j, m_informations->m_voldata[m_informations->m_frmIdx * m_informations->m_szFrm + i +
                                                        j * m_informations->m_header.height]);
      }
    }
    imageToWrite->setTransducerSettings(m_transducerSettings);
  ...
  /*m_informations->m_data->setTimestamp(m_informations->m_frmTime - m_informations->m_deviceTime +
  m_informations->m_localTime);
