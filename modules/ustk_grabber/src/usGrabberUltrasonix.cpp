...
usGrabberUltrasonix::usGrabberUltrasonix() : m_communicationInormations()
{
...
{
  if (m_communicationInormations.m_voldata)
    free(m_communicationInormations.m_voldata);
  if (m_communicationInormations.m_frmInf)
...
void usGrabberUltrasonix::start()
{
  ...
  // Bind.
  if (bind(m_communicationInormations.m_serv_fd, (struct sockaddr *)&m_communicationInormations.m_serv_addr,
           sizeof(m_communicationInormations.m_serv_addr)) < 0) {
    std::cerr << "Error: in usGrabberUltrasonix::start(): "
  ...
  // Accept.
  std::cout << "Waiting for connection, please start sending data from the Ultrasonix device." << std::endl;
  m_communicationInormations.m_clilen = sizeof(m_communicationInormations.m_cli_addr);
  m_communicationInormations.m_cli_fd =
      accept(m_communicationInormations.m_serv_fd, (struct sockaddr *)&m_communicationInormations.m_cli_addr,
             &m_communicationInormations.m_clilen);
  ...
  std::cout << std::endl
            ...
            << "   frameRate = " << m_communicationInormations.m_header.framerate << std::endl
            << std::endl;
  ...
  if (m_communicationInormations.m_header.imageType == 0) {
    ...
    m_communicationInormations.m_szFrm =
        (m_communicationInormations.m_header.width * m_communicationInormations.m_header.height *
         (m_communicationInormations.m_header.sampleSize / 8));
    ...
    m_communicationInormations.m_szFrm = m_communicationInormations.m_header.width *
                                         m_communicationInormations.m_header.height * 2; // RF = short = 2 bytes
  } else {
    std::cerr << "Error: in usGrabberUltrasonix::start(): "
  ...
  m_communicationInormations.m_szVol =
      m_communicationInormations.m_szFrm * m_communicationInormations.m_header.framesPerVolume;
  m_communicationInormations.m_voldata =
      (unsigned char *)malloc(m_communicationInormations.m_szVol * sizeof(unsigned char));

  // frmIdx + frmTime + frmData
  m_communicationInormations.m_byteSizeFrmInf =
      sizeof(int) + sizeof(double) + m_communicationInormations.m_szFrm * sizeof(unsigned char);
  ...
  for (int i = 0; i < NUM_FRAME_OFFSET; ++i) {
    ...
    while (n < m_communicationInormations.m_byteSizeFrmInf) {
      ssize_t len = recv(m_communicationInormations.m_cli_fd, m_communicationInormations.m_frmInf + n,
                         m_communicationInormations.m_byteSizeFrmInf - n, 0);
      if (len == 0) {
        std::cerr << "Error: in usGrabberUltrasonix::start(): "
                  << "recv() returned value 0: "
                  << "No messages are available to be received "
  ...
  if (m_communicationInormations.m_header.imageType == 0) // Prescan
  {
    ...
    {
      m_transducerSettings =
          usTransducerSettings(m_communicationInormations.m_header.ProbeRadius / 1000000.0,
                               vpMath::rad(US_4DC7_DEG_PER_LINE), m_communicationInormations.m_header.width, true,
                               US_4DC7_BSAMPLE_DISTANCE * m_communicationInormations.m_header.height);
    } else // 3D
    {
      ...
      m_motorSettings =
          usMotorSettings(m_communicationInormations.m_header.MotorRadius / 1000000.0,
                          vpMath::rad(m_communicationInormations.m_header.degPerFr / 1000.0),
  ...
  } else if (m_communicationInormations.m_header.imageType == 1) // Postscan
  {
    ...
    } else // 3D
    {
      ...
      m_motorSettings = usMotorSettings(m_communicationInormations.m_header.MotorRadius / 1000000.0,
                                        vpMath::rad(m_communicationInormations.m_header.degPerFr / 1000.0), 1,
                                        usMotorSettings::TiltingMotor);
    }
  } else if (m_communicationInormations.m_header.imageType == 2) // RF
  {
    std::cerr << "Error: in in usGrabberUltrasonix::start(): "
              << "Handling of data type " << m_communicationInormations.m_header.imageType
              << " (RF) is not implemented." << std::endl;
    exit(EXIT_FAILURE);
