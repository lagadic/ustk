...
double computePhotometricMoment(const vpImage<unsigned char> &I, const vpImage<unsigned char> &M, int p, int q)
{
  unsigned int height(I.getHeight()), width(I.getWidth());
  double m = 0.0;
  for (unsigned int x = 0; x < height; ++x)
...
void computeBarycenter(const vpImage<unsigned char> &I, const vpImage<unsigned char> &M, double &xc, double &yc)
{
  unsigned int height(I.getHeight()), width(I.getWidth());
  double I_sum = 0.0;
  xc = 0.0;
...
void extract(const vpImage<unsigned char> &Src, vpImage<double> &Dst, const usRectangle &r)
{
  ...
  for (unsigned int x = 0; x < x_d; ++x)
    for (unsigned int y = 0; y < y_d; ++y)
      Dst(x, y,
          interpolate(Src, x1 + x * cos(t) + y * sin(t), y1 - x * sin(t) + y * cos(t), usImageMathematics::BILINEAR));
}
}
