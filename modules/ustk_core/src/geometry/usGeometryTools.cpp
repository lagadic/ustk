...
bool DoesSegmentCrossPlane(const usBSpline3D &spline, const usOrientedPlane3D &plane)
{
    ...
    return false;*/
    return ( IsPointInFrontOfPlane(spline.accessSegment(0).getStartPoint(), plane) != IsPointInFrontOfPlane(spline.accessLastSegment().getEndPoint(), plane) );
}
...
vpColVector getPlaneCurveCrossingPoint(const usPolynomialCurve3D &poly, const usOrientedPlane3D &plane, double threshold, double *t)
{
    if(threshold <= 0) threshold = 1e-4 * poly.getParametricLength();
    ...
    if( sideStart == sideEnd ) throw vpException(vpException::badValue, "usGeometryTools::getPlaneCurveCrossingPoint(usPolynomialCurve3D): segment and plane do not cross");

    if(t) *t = mid;
    return poly.getPoint(mid);
}

vpColVector getPlaneCurveCrossingPoint(const usBSpline3D &spline, const usOrientedPlane3D &plane, double threshold, double *t)
{
    int seg = 0;
    double L = 0;
    int N = spline.getNbSegments();
    ...
    if(N>1)
    {
        ...
        for(int i=0 ; i<seg ; i++) L += spline.accessSegment(i).getParametricLength();
    }/**/

    if(seg >= N)
    {
        throw vpException(vpException::badValue, "usGeometryTools::getPlaneCurveCrossingPoint(usBSpline3D): spline and plane do not cross");
    }
    else
    {
        double t1 = -1;
...
{
    if(point.size() != 3) throw vpException(vpException::dimensionError, "usGeometryTools::projectPointOnCurve: invalid point dimension");

    double start = poly.getStartParameter();
    double stop = poly.getEndParameter();

    if(threshold <= 0) threshold = fabs(stop - start)/1000;
...
vpColVector projectPointOnCurve(const vpColVector &point, const usBSpline3D &spline, double threshold, int *index, double *t)
{
    if(point.size() != 3) throw vpException(vpException::dimensionError, "usGeometryTools::projectPointOnCurve: invalid point dimension");
    ...
    if(index) *index = seg;
    if(seg == 0) return projectPointOnCurve(point, spline.accessSegment(seg), threshold, t);
...
usPolynomialCurve3D convertBSplineToPolynomial(const usBSpline3D &spline, int order)
{
    ...
    std::vector<double> l;
    for(int i=0 ; i<nbPoints ; i++) p.push_back(spline.getPoint(i * L / nbSegments));
    l.push_back(0);
    for(int i=0 ; i<nbSegments ; i++) l.push_back(l.back() + (p.at(i+1)-p.at(i)).euclideanNorm());
...
usBSpline3D convertPolynomialToBSpline(const usPolynomialCurve3D &poly, int nbSegments, int order)
{
    ...
    std::vector<vpColVector> p;
    std::vector<double> l;
    for(int i=0 ; i<nbPoints ; i++) p.push_back(poly.getPoint(i * L / nbSegments));
    for(int i=0 ; i<nbSegments ; i++) l.push_back((p.at(i+1)-p.at(i)).euclideanNorm());
...
vpPoseVector findPointCloudRelativePose(std::vector<vpColVector> &P1, std::vector<vpColVector> &P2, double *res)
{
    ...
    if(nbPoints<2) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativePose: not enough points");
    if(nbPoints != P2.size()) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativePose: non matching number of points");
...
vpTranslationVector findPointCloudRelativePosition(std::vector<vpColVector> &P1, std::vector<vpColVector> &P2,  const vpRotationMatrix &worldRprobe, double *res)
{
    unsigned int nbPoints = P1.size();
    if(nbPoints<2) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativePose: not enough points");
    if(nbPoints != P2.size()) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativePose: non matching number of points");
...
{
    unsigned int nbPoints = P1.size();
    if(nbPoints<2) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativeRotation: not enough points");
    if(nbPoints != P2.size()) throw vpException(vpException::dimensionError, "usGeometryTools::findPointCloudRelativeRotation: non matching number of points");

    vpMatrix H(3,3,0);
    for(unsigned int i=0 ; i<nbPoints ; i++) H += P1.at(i) * P2.at(i).t();
...
vpPoseVector ICPPointCloudRelativePose(std::vector<vpColVector> &P1, std::vector<vpColVector> &P2, double *res, vpPoseVector *initialGuess)
{
    if(P1.size()<1 || P2.size()<1) throw vpException(vpException::dimensionError, "usGeometryTools::ICPPointCloudRelativePose: not enough points to match");
    ...
    else
    {
        ...
        for(unsigned int i=0 ; i<P1.size() ; i++) meanP1 += P1[i];
        meanP1 /= (double)P1.size();
        vpColVector meanP2(3,0);
        for(unsigned int i=0 ; i<P2.size() ; i++) meanP2 += P2[i];
    ...
    while( (it < 10) && (resRatio > 0.01) )
    {
        ...
        if(residual > 0) resRatio = fabs(residual - resi) / residual;
        else resRatio = 1;
        residual = resi;
    }
...
vpTranslationVector ICPPointCloudRelativePosition(std::vector<vpColVector> &P1, std::vector<vpColVector> &P2, const vpRotationMatrix &worldRprobe, double *res, vpTranslationVector *initialGuess)
{
    throw vpException(vpException::notImplementedError, "usGeometryTools::ICPPointCloudRelativePosition: non implement yet");
}

vpRotationMatrix ICPPointCloudRelativeRotation(std::vector<vpColVector> &P1, std::vector<vpColVector> &P2, double *res, vpRotationMatrix *initialGuess)
{
    throw vpException(vpException::notImplementedError, "usGeometryTools::ICPPointCloudRelativeRotation: non implement yet");
}
...
bool fitCircleTo2DPointCloud(const std::vector<vpColVector> &points, vpColVector &center, double &r)
{
    ...
    if(nbPoints < 3) throw vpException(vpException::ioError, "usGeometryTools::fitCircleTo2DPointCloud: not enough points");

    // Create data matrix with centered vectors
    vpMatrix M(3,nbPoints);
    vpColVector mean(2,0);

    for(unsigned int i=0 ; i<nbPoints; i++) mean += points.at(i);
    mean /= nbPoints;
