...
void usPolynomialCurve2D::setControlPoints(const vpMatrix &controlPoints)
{
  ...
  if ((m_controlPoints.getCol(m_order - 1) - m_controlPoints.getCol(0)).euclideanNorm() == 0.0) {
    std::cerr << "Warning: in usPolynomialCurve2D::setControlPoints(): needle model is degenerate." << std::endl;
    ...
    if (m_order != 6) {
      vpMatrix tapprox =
          usNeedleDetectionTools::approximateCoordinates(m_controlPoints.t(), m_controlPoints.t(), m_order);
      ...
      if (norm == 0.0) {
        ...
        if (t == 0.0) {
          ...
          for (unsigned int i = 0; i < 2; ++i) {
            m_model[i][0] = m_controlPoints[i][0];
            m_model[i][2] =
                (t * (m_controlPoints[i][2] - m_model[i][0]) - m_controlPoints[i][1]) / (t * (1.0 - t) + 1e-10);
