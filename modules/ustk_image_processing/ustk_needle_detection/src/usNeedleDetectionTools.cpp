...
vpColVector usNeedleDetectionTools::geometricMedian(vpMatrix points, unsigned int npts, unsigned int d)
{
  if (npts == 0) {
...
bool diff(int i, int j) { return (i != j); }
...
void usNeedleDetectionTools::computeQuantile(unsigned char *data, unsigned int dataSize, unsigned int nDesired,
                                             unsigned int &q, unsigned int &nThresholded)
{
...
void usNeedleDetectionTools::computeQuantile(short *data, unsigned int dataSize, unsigned int nDesired, unsigned int &q,
...
void usNeedleDetectionTools::computeQuantile(const unsigned int *data, unsigned int dataSize, unsigned int nDesired,
                                             unsigned int &q, unsigned int &nThresholded)
{
  unsigned int max = 0;
  for (unsigned int i = 0; i < dataSize; ++i)
                                       ...
                                       const vpColVector &origin, const vpColVector &entryPlane, int VOI[6])
{
  ...
  if (nPoints == 2) {
    found = intersectionLinePlane(model, entryPlane, -origin[0] * entryPlane[0] - origin[1] * entryPlane[1], entry[0],
                                  entry[1], entry[2]);
    return found;
  }
...
void usNeedleDetectionTools::linearRegression(vpMatrix &points, double &Xm, double &Ym, double &Zm, double &u,
                                              double &v, double &w)
{
...
void usNeedleDetectionTools::computeControlPoints(const vpMatrix &model, vpMatrix &mss, double **points,
...
vpMatrix usNeedleDetectionTools::approximateCoordinates(vpMatrix X, vpMatrix MSS, unsigned int s)
{
...
bool usNeedleDetectionTools::intersectionLinePlane(vpMatrix line, const vpColVector &plane, double offset, double &x,
                                                   double &y, double &z)
{
