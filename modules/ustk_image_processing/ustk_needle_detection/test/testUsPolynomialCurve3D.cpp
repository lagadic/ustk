...
int main()
{
    ...
    points2.at(1)[0] = 1;points2.at(2)[1] = 2;points2.at(3)[2] = 1;
    std::vector<double> params2(4, 0);
    for(int i=0 ; i<4 ; i++) params2[i] = i;
    
    std::vector<vpColVector> points3(10, vpColVector(3,0));
    points3.at(1)[0] = 1;points3.at(2)[1] = 2;points3.at(3)[2] = 3;
    points3.at(4)[1] = 1;points3.at(5)[1] = 2;points3.at(6)[2] = 3;
    points3.at(7)[0] = 1;points3.at(8)[0] = 2;points3.at(9)[0] = 2;
    points3.at(7)[1] = 2;points3.at(8)[1] = 2;points3.at(9)[2] = 2;
    ...
    std::cout << "done: case 1 usPolynomialCurve3D::defineFromPoints(const std::vector<vpColVector> &points, const std::vector<double> &param, unsigned int order)" << std::endl;
    curve.defineFromPoints(points2, params2, 4);
    std::cout << "done: case 2 usPolynomialCurve3D::defineFromPoints(const std::vector<vpColVector> &points, const std::vector<double> &param, unsigned int order)" << std::endl;
    curve.defineFromPoints(point3M, params3V, 3);
    std::cout << "done: case 3 usPolynomialCurve3D::defineFromPoints(const std::vector<vpColVector> &points, const std::vector<double> &param, unsigned int order)" << std::endl;
    
    curve.defineFromPoints(points3, params3, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromPoints(const vpMatrix points, const vpColVector &param, unsigned int order)" << std::endl;
    curve.defineFromPointsAuto(points3, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromPointsAuto(const std::vector<vpColVector> &points, unsigned int order)" << std::endl;
    curve.defineFromPointsAuto(point3M, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromPointsAuto(const vpMatrix &points, unsigned int order)" << std::endl;
    
    vpColVector direction(3,0);
    direction[0] = 1;
    curve.defineFromPointsAuto(points3, direction, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromPointsAuto(const std::vector<vpColVector> &points, const vpColVector &direction, unsigned int order)" << std::endl;
    curve.defineFromPointsAuto(point3M, direction, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromPointsAuto(const vpMatrix &points, const vpColVector &direction, unsigned int order)" << std::endl;
    curve.defineFromWeightedPoints(points3, params3, weights3,3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPoints(const std::vector<vpColVector> &points, const std::vector<double> &param, const std::vector<double> &weights, unsigned int order)" << std::endl;
    curve.defineFromWeightedPoints(point3M, params3V, weights3V, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPoints(const vpMatrix &points, const vpColVector &param, const vpColVector &weights, unsigned int order)" << std::endl;

    curve.defineFromWeightedPointsAuto(points3, weights3, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPointsAuto(const std::vector<vpColVector> &points, const std::vector<double> &weights, unsigned int order)" << std::endl;
    curve.defineFromWeightedPointsAuto(point3M, weights3V, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPointsAuto(const vpMatrix &points, const vpColVector &weights, unsigned int order)" << std::endl;
    curve.defineFromWeightedPointsAuto(points3, weights3, direction, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPointsAuto(const std::vector<vpColVector> &points, const std::vector<double> &weights, const vpColVector &direction, unsigned int order)" << std::endl;
    curve.defineFromWeightedPointsAuto(point3M, weights3V, direction, 3);
    std::cout << "done: usPolynomialCurve3D::defineFromWeightedPointsAuto(const vpMatrix &points, const vpColVector &weights, const vpColVector &direction, unsigned int order)" << std::endl;
