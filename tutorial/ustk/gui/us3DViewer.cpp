...
int main(int argc, char **argv)
{
  ...
  importer->SetWholeExtent(0, postScan3D.getWidth() - 1, 0, postScan3D.getHeight() - 1, 0,
                           postScan3D.getNumberOfFrames() - 1);
  ...
  planeX->SetOrigin(postScan3D.getWidth() * spacing[0] / 2.0, 0, 0);
  vtkSmartPointer<vtkPlane> planeY = vtkSmartPointer<vtkPlane>::New();
  planeY->SetNormal(0, 1, 0);
  planeY->SetOrigin(0, postScan3D.getHeight() * spacing[1] / 2.0, 0);
  vtkSmartPointer<vtkPlane> planeZ = vtkSmartPointer<vtkPlane>::New();
  planeZ->SetNormal(0, 0, 1);
  planeZ->SetOrigin(0, 0, postScan3D.getNumberOfFrames() * spacing[2] / 2.0);
